...
전류를 많이 넣어주면, 자속이 증가하는 것을 알수 있다. 이때 일정하게 자속이 증가하는 부분을 선형 영역이라 하한다.  
...
이때 회전자와 고정자 사이에 공극이 감소하게 되고 자기 저항 성분이 감소하고 더 큰 쇄교 자속이 발생해, 인덕턴트가 증가한다는 의미를 갖는다.  
...
기계적인 힘은 자기 회로의 인덕턴스가 증가하는 방향, 자기 저항이 감소하는 방향으로 증가 한다는 결과를 알 수 있다.  
