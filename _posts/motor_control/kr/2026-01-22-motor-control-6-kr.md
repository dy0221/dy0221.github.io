...
1. 1 오른손 법칙은 도체가 움직일때 기준이다. 따라서 자석이 $$v$$방향으로 움직인다면, 자계는 멈춰있고 자신이 자계가 움직이는 반대인 $$-v$$방향 으로 움직인다고 생각 할수 있다. 이를 통해 플레밍의 오른손 법칙을 적용하면, 회전자의 오른쪽 부분을 보면, 유도 기전력이 뚫는 방향으로 생성된다는 것을 알 수 있다.  
...
<p align="center">
  <img src="/assets/images/motor_control/6/3sang_mmf.png" width="650px"/>
  <br/>
  <strong>그림 7.</strong> t_0일때, 기자력의 분포
</p>
  
그림 6과 그림 7을 기준으로 $$t_0$$일 때의 상황을 살펴보자. 그림7의 왼쪽 그림이나 가운데 그림 어느 곳을 보더라도, 3상 권선에서 발생하는 기자력의 합을 보면 하나의 기자력으로 볼 수 있고, 이는 N극과 S극이 하나씩있는 자석이라고 생각할 수 있다.  
  
또한 $$t_0$$일 때, 전류가 a상이 $$I_m$$으로 가장 크고 b와 c상이 $$-0.5I_m$$으로 같은 것을 알 수 있다. 따라서 기자력 또한 a상이 가장 크고, b,c상이 같은 것을 알 수 있다.  
  
교류 전류 1주기 동안 합성 기자력 $$F$$의 공간적 움직임을 살펴보면, a상->b상->c상의 축을 따라서 1회전을 한다. 
<p align="center">
...
회전 자계의 속도를 **동기속도(Synchronous Speed, $$n_s$$)**라고 하는데, 극수(P, Pole)와 주파수($$f$$)에 의해 결정된다. 따라서 이는 정해지면 변하지 않는다.  
  
우리가 위에서 본 회전자계는 N극 S극 하나씩 있으므로 2극이라고 표현한다, (2개씩 있으면 4극 ...)  
<p align="center">
$$  
...
합성기자력의 식을 분석해보면 크기는 한개의 상의 최대 기자력의 1.5배($$\frac{3}{2}N_s I_m$$)가 된다. 또한 어느 시점t에서는 공극의 위치$$\theta$$에 따라 기자력이 정현파의 형태로 분포 함을 알 수 있다.($$F(\theta) \simeq F_{max}\cos \theta$$)  
  
기자력의 최댓값은 $$\omega_s t = \theta$$인 위치에서 나타나는데, 이는 시간의 흐름에 따라 $$\omega_s$$의 속도로 이동하는 것을 의미한다. 따라서 합성 기자력은 전류의 각속도로 회전하는 회전자계를 형성한다.  
...
유도된 자속에 의해 만들어진 전압은 자속이 공극을 통해 회전자를 지나가 회전자에 전압을 유도하는 자속($$\phi_m$$)도 있을 것이고, 누설되어 고정자 철심에만 머무는 누설 자속($$\phi_{ls}$$) 또한 있을 것이다. 이를 등가회로로 표현하면 다음과 같다.  
...
간단하게 설명하면, 고정자 권선에 전류 $$I_s$$를 흘려보내면, 고정자 권선에 있는 철심을 자화 시키기 위한 여자 전류($$I_{\phi}$$)와 회전자에서 사용하는 전류 $$I_r$$을 고정자 측에서 환산한 $$I_r'$$으로 나누어 져서 사용된다.  
...
고정자를 보면, 각 상은 실제로는 슬롯에 정현적으로 분포되어 있다.(그림 4) 따라서 실제 기전력은 각 상의 중첩 때문에, 이론적 값보다 10%정도 작게 나오게 된다. 이를 고려 해서 $$K_{ws}$$라는 권선 계수를 사용해서 유기 기전력을 표시하게 된다.  
...
상대 속도는 동기 속도와 실제 속도의 차이로 구할 수 있다. 이를 바탕으로 슬립은 **동기속도에 대한 상대속도의 비**로 정의 할 수 있다.  
<p>
\begin{align}
sn_s &= n_s -n \\[4pt]
...
1. **동기속도는 항상 실제속도보다 빠르다** (회전자계에 의해 회전자가 끌려오기 때문)  
   그렇기 때문에 슬립 s는 0 ~ 1 사이의 값을 가진다.

   - 회전자가 멈춰있는 경우(정지 시)
     실제 속도 $$n = 0$$ 이므로 **s = 1**이다.

   - 실제 속도가 동기속도와 같은 경우
     (실제로는 같지 않으므로 매우 가까운 경우)  
     $$n \simeq n_s$$ 이고 **s = 0**이다.

2. 실제로 정상 운전 시 모터의 슬립은 **0.03 ~ 0.05** 사이로 매우 작다.

슬립이라는 것은 회전자계와 회전자의 속도 차이를 나타내는 비율이다. 회전자계는 전기각속도 $$\omega_s$$로 회전하고 있고, $$\omega_s$$는 전원의 인가 주파수(또는 전류 주파수) $$f_s$$에 의해 결정된다.($$\omega_s = 2\pi f_s$$)   
  
회전자가 회전자계보다 s만큼의 비율로 느리게 돈다면, 그 비율 만큼 회전자가 회전자계의 자속을 끊게 된다.(주파수)  

ex)
1. 고정자의 회전자계가 1초에 100번 회전한다 -> 동기속도 $$n_s = 100 [\text{rps}]$$이 될것이고, 고정자는 가만히 있기 때문에 회전자계가 회전하는데로 1초에 100번 자속을 끊게 된다. 따라서 고정자 권선에 유도 되는 기전력은 전원 주파수 그대로 $$f_s = 100hz$$를 가지게 된다.    
...
3. 이를 일반화 하면 고정자의 회로의 전원 주파수 $$f_s =100(\text{hz})$$가 있는데, 회전자의 회로에 유도되는 전류의 주파수는 $$s = \frac{100-80}{100}= 0.2$$의 비율로 변화한다. $$f_r = sf_s = 0.2 \times 100 = 20(\text{hz})$$
...
회전자 회로의 주파수$$f_r$$은 회전자가 정지시에 가장 크다.($$f_r=f_s$$가 되기 때문에) 이때 유기 기전력을 $$E_{r0}$$라고 한다.  
...
일반적인 상황(운동시)에서는 유도 기전력은 정지시의 **s배**가 되게 된다.  
<p align="center">
$$  
...
이처럼 유도전동기에서 회전자에서 주파수와 관련된 값은 전부 운동시, 정지시의 s배 이다.  
...
\rightarrow & E_s =aE_{r0} \, , \, I_s = \frac{I_r}{a} \\[4pt] 
...
회전자와 고정자는 연결되어 있지 않는다. 하지만 고정자 측에서 볼때 회전자 값을 환산했다고 해서 **'**을 붙인다.  
  
마지막으로 알아야 할것은 회전자가 만드는 회전자계이다.  
  
고정자의 3상 권선에 교류전류를 흘려 보내 만들어진 자속이 회전자계$$F_s$$를 만들었다면, 회전자의 3상 권선에 유도된 전류 또한 회전자계 $$F_r$$를 만든다.  
  
회전자를 기준으로 살펴보면 이 회전자에의한 회전자계는 회전자의 주파수인 상대속도 $$sn_s (=n_s - n)$$으로 회전한다.  
  
이를 멈춰있는 고정자를 기준으로 살펴보면 회전자가 돌이가는 실제 속도 $$n$$과 상대속도 $$sn_s$$를 합친 만큼의 속도로 돌아간다는 것을 알 수 있고, 이는 고정자가 만드는 회전자계$$F_s$$의 속도인 동기속도 $$n_s(= sn_s + n)$$과 같은 것을 알수 있다.  
